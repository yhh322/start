...
void test0()
{
	//multiset中默认情况下按升序的方式进行排列
	//multiset中可以存放关键字相同的元素 
	multiset<int> numbers{1, 1, 6, 6, 2, 3, 5, 1, 6, 4};
	display(numbers);
	...
	size_t count = numbers.count(1);//o(logN)
	...
	//范围查找
	auto it3 = numbers.lower_bound(1);//下限 [
	auto it4 = numbers.upper_bound(1);//上限 )
	while(it3 != it4) {
		cout << *it3 << " ";
		++it3;
	}
	...
	auto ret = numbers.equal_range(6);
...
void test2()
{
	...
	multiset<Point, PointComparator> numbers{
		Point(1, 2),
		Point(3, 4),
		Point(1, 2),
		Point(5, 6),
	...
	//multiset底层实现是红黑树 --> 近似平衡二叉树 ---> 访问某一个节点的时间复杂度为O(logN)
	//特点:
	//	1. 节点不是红色就是黑色
	//	2. 根节点是黑色的
	//	3. 所有叶子节点是黑色
	...
	//如果只要判断multiset中是否包含某个元素，使用count
